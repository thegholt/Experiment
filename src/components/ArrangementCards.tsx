...
function SourceLinks({ sourceIds }: { sourceIds: string[] }) {
  ...
  if (!sources.length) return <p className="text-sm text-slate-500">No source links recorded.</p>;
  ...
  return (
    <ul className="mt-2 list-disc space-y-1 pl-5 text-sm text-slate-600">
      {sources.map((source) => (
        <li key={source.id}>
          {source.url ? (
            <a className="underline hover:text-slate-950" href={source.url}>
              {source.title}
            </a>
          ) : (
            source.title
    ...
    <article className="rounded-xl border border-slate-200 bg-white p-6 shadow-sm">
      <div className="flex flex-wrap items-start justify-between gap-4">
        <div>
          ...
          <h2 className="mt-1 text-2xl font-semibold text-slate-950">{area.name}</h2>
          <p className="mt-2 text-sm text-slate-600">{area.areaType}</p>
        </div>
      ...
      <div className="mt-6 grid gap-4 rounded-lg border border-slate-200 bg-slate-50 p-4">
        <div>
          <h3 className="text-sm font-semibold text-slate-900">Summary reasoning</h3>
          <p className="mt-2 text-sm leading-6 text-slate-600">{area.score.summaryReasoning}</p>
        </div>
        <div>
    ...
    <article className="rounded-xl border border-slate-200 bg-white p-6 shadow-sm">
      <div className="flex flex-wrap items-start justify-between gap-4">
        <div>
          ...
          <h2 className="mt-1 text-2xl font-semibold text-slate-950">{parliament.name}</h2>
          <p className="mt-2 text-sm text-slate-600">{parliament.country}</p>
        </div>
      ...
      <div className="mt-6 rounded-lg border border-slate-200 bg-slate-50 p-4">
        <h3 className="text-sm font-semibold text-slate-900">Summary reasoning</h3>
        <p className="mt-2 text-sm leading-6 text-slate-600">{parliament.score.summaryReasoning}</p>
        <h3 className="mt-4 text-sm font-semibold text-slate-900">Sources</h3>
